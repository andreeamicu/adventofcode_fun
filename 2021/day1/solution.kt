...
fun main() {
	testSamplePart1()
	testSamplePart2()
	val measures = readInput("input.txt")
	val result1 = measures.increaseCount()
	println("Answer part1: $result1")
	val result2 = measures.sumIncreaseCount()
	println("Answer part2: $result2")
}

private fun List<Int>.increaseCount(): Int {
	//println("Measurement changes:" + changes().toString())
	return changes().count { isIncrease -> isIncrease }
}

private fun List<Int>.changes(): List<Boolean> {
	return zipWithNext { a, b -> b > a }
}

private fun List<Int>.sumIncreaseCount(): Int {
	return windowed(3) { it.sum() }.increaseCount()
}
...
private fun testSamplePart1() {
	...
	val resultActual = readSample().increaseCount()
...
private fun testSamplePart2() {
	val resultExpected = 5
	val resultActual = readSample().sumIncreaseCount()
	if(resultExpected != resultActual) {
